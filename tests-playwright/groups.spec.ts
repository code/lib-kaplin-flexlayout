...
import { drag, findAllTabSets, findPath, findTabButton, Location, waitForBox } from "./helpers";
...
const expectTabButtonCount = async (page: import("@playwright/test").Page, path: string, count: number) => {
    await expect(findPath(page, path).locator('[role="tab"]')).toHaveCount(count);
};
...
test.describe("grouped tabs", () => {
    ...
    test("renders group pills and their tab buttons", async ({ page }) => {
        ...
        await expect(findPath(page, "/ts0/g0/tb0")).toContainText("Colors");
        await expect(findPath(page, "/ts0/g0/tb1")).toContainText("Typography");
        await expect(findPath(page, "/ts0/g0/tb2")).toContainText("Icons");
        ...
        await expect(findPath(page, "/border/right/g0/tb0")).toContainText("Theme");
        await expect(findPath(page, "/border/right/g0/tb1")).toContainText("Layout JSON");
    });
    ...
    test("tab context menu can add a tab to a new group", async ({ page }) => {
        ...
        await expect(findPath(page, "/ts0/g3/tb0")).toContainText("Settings");
    });

    test("add to new group moves a grouped tab out of its existing group", async ({ page }) => {
        // Colors (in the Design group) moving to a new group must not duplicate it
        await findPath(page, "/ts0/g0/tb0").click({ button: "right" });
        await expect(page.locator(".flexlayout__popup_menu")).toBeVisible();
        await page.getByRole("menuitem", { name: "Add to new group", exact: true }).click();
    ...
    test("tab context menu can add a tab to an existing group", async ({ page }) => {
        ...
        await expectTabButtonCount(page, "/ts0", 4);
        await expect(page.locator('[role="tab"]', { hasText: "Settings" })).toHaveClass(/flexlayout__tab_button_grouped/);
    });

    test("dragging a tab out of a group moves it to the tabset", async ({ page }) => {
        // drag Colors (inside Design) onto Settings -> Colors leaves the Design group
        // and becomes an ungrouped tab
        const from = findPath(page, "/ts0/g0/tb0");
        ...
        await expect(findPath(page, "/ts0/g0/tb0")).toContainText("Typography");
        await expect(findPath(page, "/ts0/g0/tb1")).toContainText("Icons");
    ...
    test("dragging a tab into a group adds it to the group", async ({ page }) => {
        ...
        await findPath(page, "/ts0/g1").click();
        await expect(findPath(page, "/ts0/g1/tb1")).toBeVisible();
        // drag Monthly (in Reports) onto the Design pill -> Monthly joins Design at its start
        const from = findPath(page, "/ts0/g1/tb1");
        const to = findPath(page, "/ts0/g0");
        await drag(page, from, to, Location.CENTER);
        await expectTabButtonCount(page, "/ts0", 6); // Design(4) + Reports(1) + Settings
        await expect(findPath(page, "/ts0/g0/tb0")).toContainText("Monthly");
        await expect(findPath(page, "/ts0/g0/tb1")).toContainText("Colors");
    });
    ...
    test("ungrouping a group returns its tabs to the tabset", async ({ page }) => {
        ...
        await expect(findPath(page, "/ts0/tb1")).toContainText("Weekly");
    });
    ...
    test("dragging a tab into the right border group adds it", async ({ page }) => {
        // drag Settings (ungrouped, clear of the right border) onto the Helpers pill
        ...
        await expect(findPath(page, "/border/right/g0/tb0")).toContainText("Settings");
    });
    ...
    test("reordering border tabs shows a horizontal drop indicator", async ({ page }) => {
        const src = (await findPath(page, "/border/right/g0/tb1").boundingBox())!; // Layout JSON
        const tgt = (await findPath(page, "/border/right/g0/tb0").boundingBox())!; // Theme
    ...
    test("dragging within a group works after switching to the underline type", async ({ page }) => {
        ...
        // drag Colors onto the left edge of Icons (both in the Design group)
    ...
    test("a drop in the gap after the last group's tab lands on the strip after switching to underline", async ({ page }) => {
        ...
        // drag Colors into the empty space between Design's last tab (Icons) and the Reports
        ...
        const icons = (await findPath(page, "/ts0/g0/tb2").boundingBox())!;
