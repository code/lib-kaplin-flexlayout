...
test("MUI component is styled in a popout and reacts to dynamic style changes", async ({ page, context }) => {
    ...
    const sliderColor = await popout.evaluate(() => getComputedStyle(document.querySelector(".MuiSlider-root")!).color);
    expect(sliderColor).toBe("rgb(25, 118, 210)"); // MUI primary blue
    ...
    const after = await popout.evaluate(() => getComputedStyle(document.querySelector(".MuiSlider-root")!).color);
    expect(after).toBe("rgb(46, 125, 50)"); // MUI success green
});
...
test("styled-components component is styled in a popout", async ({ page, context }) => {
    ...
    const boxBg = await popout.evaluate(() => {
    ...
    expect(boxBg).toBe("rgb(25, 118, 210)"); // the styled Box background
});
